...
public sealed class WimDeploymentService
{
        ...
        out string error)
    {
        ImagingPartitionInfo? efi = disk.Partitions.FirstOrDefault(static partition =>
            partition.StorageInfo.IsEfiSystem);
        if (efi != null)
        {
            systemPartition = efi;
            firmwareType = "UEFI";
        ...
        bool looksGpt = string.Equals(
            disk.StorageInfo.PartitionStyle,
            "GPT",
        ...
        ImagingPartitionInfo? active = looksGpt
            ...
            : disk.Partitions.FirstOrDefault(static partition =>
                partition.StorageInfo.IsActive == true);
        if (active != null)
        {
        ...
        error =
            $"Windows Imaging Utility could not identify a boot system partition on Disk {disk.DiskNumber}. " +
