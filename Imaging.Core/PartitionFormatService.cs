...
public sealed class PartitionFormatService
{
    ...
    public PartitionFileSystemResult GetCurrentFileSystem(string targetRoot)
    {
        ...
        try
        {
            ...
            if (!SupportedFileSystems.Contains(fileSystem))
            {
                return PartitionFileSystemResult.Failed(
                    $"The selected partition uses the unsupported filesystem '{fileSystem}'. Windows Imaging Utility will not guess a replacement filesystem before applying the WIM.");
            }
        ...
        {
            return PartitionFileSystemResult.Failed(
                "Windows Imaging Utility could not determine the current filesystem of the selected partition. " +
        ...
        CancellationToken cancellationToken)
    {
        ...
        string markerPath = Path.Combine(root, $".WindowsImagingUtility-FormatVerify-{Guid.NewGuid():N}.tmp");
        try
        {
            await File.WriteAllTextAsync(
                markerPath,
                "Windows Imaging Utility format verification marker.",
                Encoding.ASCII,
        ...
        {
            return PartitionFormatResult.Failed(
                $"Windows Imaging Utility could not create a format-verification marker on {root}. " +
        ...
        try
        {
            ...
            // Give Windows a moment to refresh the newly formatted filesystem before DISM starts.
            ...
            {
                return PartitionFormatResult.Failed(
                    "The target partition was formatted, but Windows Imaging Utility could not verify the resulting filesystem.\n\n" + ex.Message,
                    result.ExitCode,
                    result.CombinedOutput);
            }
