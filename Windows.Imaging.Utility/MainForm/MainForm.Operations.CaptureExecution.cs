...
public partial class MainForm
{
        ...
        bool appendToExistingWim)
    {
            ...
            !string.Equals(operationImagePath, imagePath, StringComparison.OrdinalIgnoreCase))
        {
            if (!TryCommitTemporaryOutput(operationImagePath, imagePath, out string? commitError))
            {
                result = new WimOperationResult
                {
                    Success = false,
                    ExitCode = -1,
                    Output = "The replacement WIM was captured successfully, but Windows Imaging Utility could not safely replace the existing file.\n\n" + commitError
                };
            }
        }
        ...
        if (!string.IsNullOrWhiteSpace(cleanupError))
        {
            MessageBox.Show(
                this,
                "The capture finished, but Windows Imaging Utility could not remove the temporarily staged winre.wim from the Windows partition.\n\n" +
                cleanupError,
                "Windows RE Cleanup",
                MessageBoxButtons.OK,
                MessageBoxIcon.Warning);
