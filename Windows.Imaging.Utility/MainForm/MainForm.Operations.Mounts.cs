...
public partial class MainForm
{
    private async Task MountWimAsync()
    {
        ...
        string? mountDirectory = RunFolderPicker("Select empty folder for WIM mount");
        ...
        catch (Exception ex)
        {
            MessageBox.Show(
                this,
                $"Windows Imaging Utility could not inspect the selected mount folder.\n\n{ex.Message}",
                "Mount WIM",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error);
            return;
    ...
    private void LoadPendingWimUnmountState()
    {
        ...
        foreach (PendingWimUnmountState state in PendingWimUnmountStateStore.Load())
        {
            ...
            {
                // Ignore a malformed recovery record rather than blocking Windows Imaging Utility startup.
            }
        }
    }
        ...
        string? errorTitle = null)
    {
        ...
        if (!result.Success)
        {
            ...
            if (errorTitle != null && !IsDisposed)
            {
                ...
                MessageBox.Show(
                    this,
                    "Windows Imaging Utility could not read the mounted WIM inventory.\n\n" + details,
                    errorTitle,
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
    ...
    private async Task AddDriversAsync()
    {
        ...
        string? driverFolder = RunFolderPicker("Select driver folder to add recursively");
