...
public partial class MainForm
{
    ...
    private void UpdateSelectedDiskPanel()
    {
        ...
        bool runningWindowsDiskSelected = diskSelectionActive && WindowsRuntimeSafety.IsRunningWindowsDisk(disk);
        bool runningWindowsPartitionSelected = partitionSelectionActive && WindowsRuntimeSafety.IsRunningWindowsPartition(partition);
        ...
        _actionCaptureFfu.Enabled = actionsAvailable && diskSelectionActive && !runningWindowsDiskSelected;
        _actionApplyFfu.Enabled = actionsAvailable && diskSelectionActive && !runningWindowsDiskSelected;
        _actionDeployWim.Enabled = actionsAvailable && diskSelectionActive && !runningWindowsDiskSelected;
        _actionCaptureWim.Enabled = actionsAvailable && partitionSelectionActive && !runningWindowsPartitionSelected;
        _actionApplyWim.Enabled = actionsAvailable && partitionSelectionActive && !runningWindowsPartitionSelected;
    ...
    private string BuildDiskDetails(ImagingDiskInfo disk)
    {
        ...
        AppendInfoLine(text, "Size", FormatBytes(storage.SizeBytes));
        ...
        AppendInfoLine(text, "Partition count", storage.NumberOfPartitions.ToString());
        ...
        AppendInfoSection(text, "Partitions");
        ...
        {
            foreach (ImagingPartitionInfo partition in disk.Partitions.OrderBy(static item => item.PartitionNumber))
            {
                string type = FirstNonEmpty(partition.StorageInfo.GptType, partition.StorageInfo.MbrType, "Unknown");
                string drives = partition.DriveLetters.Count == 0
                    ? "no drive letter"
                    : string.Join(", ", partition.DriveLetters.Select(static root => root.TrimEnd('\\')));

                text.AppendLine(
                    $"Partition {partition.PartitionNumber}: {FormatBytes(partition.SizeBytes)} · {type} · {drives}");
            }
        }
    ...
    private string BuildPartitionDetails(ImagingDiskInfo? disk, ImagingPartitionInfo partition)
    {
        ...
        AppendInfoSection(text, $"Partition {partition.PartitionNumber}");
        if (disk != null)
            AppendInfoLine(text, "Disk number", disk.DiskNumber.ToString());
        AppendInfoLine(text, "Partition number", partition.PartitionNumber.ToString());
        ...
        AppendInfoLine(text, "Size", FormatBytes(storage.SizeBytes));
        AppendInfoLine(text, "Offset", FormatByteOffset(storage.OffsetBytes));
        AppendInfoLine(text, "Operational status", storage.OperationalStatus);
    ...
    private void UpdateStatusLine()
    {
        ...
        else if (WindowsRuntimeSafety.IsRunningWindowsPartition(GetSelectedPartition()))
        {
            _lblStatus.Text = WindowsRuntimeSafety.GetPartitionProtectionMessage(GetSelectedPartition()!);
            _lblStatus.Visible = true;
        }
        else if (GetSelectedPartition() == null && WindowsRuntimeSafety.IsRunningWindowsDisk(GetSelectedDisk()))
        {
            _lblStatus.Text = WindowsRuntimeSafety.GetDiskProtectionMessage(GetSelectedDisk()!);
            _lblStatus.Visible = true;
        }
