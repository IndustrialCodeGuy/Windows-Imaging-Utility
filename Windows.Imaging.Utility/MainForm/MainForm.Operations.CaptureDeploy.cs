...
public partial class MainForm
{
    private async Task CaptureSelectedPartitionWimAsync()
    {
        ...
        if (WindowsRuntimeSafety.IsRunningWindowsPartition(partition))
        {
            MessageBox.Show(
                this,
                WindowsRuntimeSafety.GetPartitionProtectionMessage(partition),
                "Capture WIM",
        ...
        string? imagePath = RunFilePicker(
            save: true,
        ...
        try
        {
            if (!TryGetPartitionCaptureRoot(partition, out sourceRoot))
            {
                ...
                if (!mountResult.Success)
                {
                    MessageBox.Show(
                        this,
                        "The selected partition does not currently have an accessible drive letter, and Windows Imaging Utility could not temporarily mount it for WIM capture.\n\n" +
                        mountResult.Error,
                        "Capture WIM",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
                ...
                if (stageChoice == DialogResult.Yes)
                {
                    ...
                    if (!stageResult.Success)
                    {
                        DialogResult continueWithoutWinRe = MessageBox.Show(
                            this,
                            "Windows Imaging Utility could not stage winre.wim from the configured Recovery partition.\n\n" +
                            stageResult.Error +
                    ...
                    {
                        stagedWinRe = stageResult.StagedByWindowsImagingUtility;
        ...
        finally
        {
            if (temporarySourceMount != null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(removalError))
                {
                    MessageBox.Show(
                        this,
                        "Windows Imaging Utility could not remove the temporary drive letter assigned to the captured partition.\n\n" +
                        removalError,
                        "Partition Cleanup",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
    ...
    private async Task DeployWimToSelectedDiskAsync()
    {
        ...
        if (WindowsRuntimeSafety.IsRunningWindowsDisk(disk))
        {
            MessageBox.Show(
                this,
                WindowsRuntimeSafety.GetDiskProtectionMessage(disk),
                "Deploy WIM",
                MessageBoxButtons.OK,
                MessageBoxIcon.Warning);
            return;
        ...
        if (firmwareType == WimDeploymentFirmwareType.Unknown)
        {
            MessageBox.Show(
                this,
                "Windows Imaging Utility could not determine whether Windows was booted in UEFI or BIOS firmware mode.\n\n" +
                "Deploy WIM currently uses the host firmware mode to choose the GPT or MBR disk layout, so deployment was not started.",
                "Deploy WIM",
                MessageBoxButtons.OK,
    ...
    private async Task<WimImageInfoResult?> TryLoadWimImageInfoAsync(string imagePath, string title)
    {
        ...
        MessageBox.Show(
            this,
            "Windows Imaging Utility could not read the image list from the selected WIM or split WIM.\n\n" + details,
            title,
            MessageBoxButtons.OK,
            MessageBoxIcon.Error);
        return null;
        ...
        WimDeploymentFirmwareType firmwareType)
    {
        ...
        try
        {
            try
            {
                // Full Windows owns C:. Deployments therefore always use a temporary
                // access letter and never move or replace the host C: assignment.
            ...
            catch (Exception ex)
            {
                MessageBox.Show(
                    this,
                    "Windows Imaging Utility could not reserve a temporary drive letter for the deployed Windows partition.\n\n" + ex.Message,
            ...
            if (!File.Exists(imagePath))
            {
                MessageBox.Show(
                    this,
                    $"The selected WIM or split WIM file is no longer accessible.\n\n{imagePath}",
                    "Deploy WIM",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
                return;
