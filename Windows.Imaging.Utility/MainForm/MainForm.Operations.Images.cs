...
public partial class MainForm
{
    ...
    private async Task SplitWimFromMenuAsync()
    {
        ...
        string? sourcePath = RunFilePicker(
            save: false,
    ...
    private async Task<bool> SplitWimAsync(string sourcePath)
    {
        ...
        string? destinationPath = RunFilePicker(
            save: true,
        ...
        if (result.Success && !result.Canceled && replacingExistingSet)
        {
            if (!TryCommitSplitWimFamily(operationDestinationPath, destinationFullPath, out string? commitError))
            {
                result = new WimOperationResult
                {
                    Success = false,
                    ExitCode = -1,
                    Output = "The replacement SWM set was created successfully, but Windows Imaging Utility could not safely replace the existing set.\n\n" + commitError
                };
            }
        }
        ...
        out string? error)
    {
        ...
        catch (Exception ex)
        {
            ...
            error = "The new SWM set could not be installed. Windows Imaging Utility attempted to restore the previous set.\n\n" + ex.Message;
