...
public partial class MainForm
{
        ...
        try
        {
            if (!TryGetPartitionCaptureRoot(partition, out targetRoot))
            {
                ...
                if (!mountResult.Success)
                {
                    MessageBox.Show(
                        this,
                        "The selected partition does not currently have an accessible drive letter, and Windows Imaging Utility could not temporarily mount it as a WIM apply target.\n\n" +
                        mountResult.Error,
                        "Apply WIM",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
            ...
            if (!fileSystemResult.Success)
            {
                MessageBox.Show(
                    this,
                    "Windows Imaging Utility could not safely prepare the selected partition for a clean WIM restore.\n\n" +
        ...
        finally
        {
            if (temporaryTargetMount != null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(removalError))
                {
                    MessageBox.Show(
                        this,
                        "Windows Imaging Utility could not remove the temporary drive letter assigned to the target partition.\n\n" +
                        removalError,
        ...
        bool configureBootFiles)
    {
        ...
        if (!formatResult.Success)
        {
            ...
            MessageBox.Show(
                this,
                "Windows Imaging Utility could not format the selected partition before applying the WIM.\n\n" +
                formatResult.Error,
