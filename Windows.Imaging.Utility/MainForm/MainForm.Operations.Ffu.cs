...
public partial class MainForm
{
    private async Task CaptureSelectedDiskAsync()
    {
        ...
        if (assessment.Suitability == FfuCaptureSuitability.BitLockerStatusUnknown)
        {
            ...
            DialogResult continueWithoutStatus = MessageBox.Show(
                this,
                "Windows Imaging Utility could not verify the BitLocker encryption state of this disk." +
        ...
        string? imagePath = RunFilePicker(save: true, title: $"Capture Disk {disk.DiskNumber} to FFU");
    ...
    private async Task ApplyToSelectedDiskAsync()
    {
        ...
        string? imagePath = RunFilePicker(save: false, title: $"Select FFU to apply to Disk {disk.DiskNumber}");
        if (string.IsNullOrWhiteSpace(imagePath))
            return;
