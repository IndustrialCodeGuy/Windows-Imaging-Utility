...
public partial class MainForm
{
    private Panel CreateDiskRow(ImagingDiskInfo disk)
    {
        ...
        partitionStrip.Resize += (_, _) => LayoutPartitionStrip(partitionStrip);
        partitionStrip.MouseDown += (_, _) => DeselectTilesFromNeutralInteraction();

        foreach (ImagingPartitionInfo partition in disk.Partitions.Where(IsPartitionVisibleInStrip))
    ...
    private static bool IsPartitionVisibleInStrip(ImagingPartitionInfo partition) =>
        !partition.StorageInfo.IsMicrosoftReserved;
    ...
    private void LayoutDiskRow(Panel row)
    {
        ...
        LayoutPartitionStrip(context.PartitionStrip);
    }
    ...
    private void LayoutPartitionStrip(FlowLayoutPanel strip)
    {
        ...
        double totalSize = tiles.Sum(static tile =>
            tile.Tag is PartitionTileContext context ? (double)context.Partition.SizeBytes : 0d);
    ...
    private Image GetDiskImage()
    {
        ...
        image = WindowsIconProvider.CreateDiskImage(size);
        if (image == null)
        {
    ...
    private Image GetPartitionImage(ImagingPartitionInfo partition)
    {
        ...
        image = WindowsIconProvider.CreatePartitionImage(kind, size);
        if (image == null)
        {
    ...
    {
        bool hasSystemVisualVolume = partition.Volumes.Any(static item => item.IsRunningSystemDrive);
